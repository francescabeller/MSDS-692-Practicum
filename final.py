...
# Get list of test file names
test_file_path = cd + r'\test_videos2'
f = open(test_file_path + '\\' + 'test_list.txt', 'r')
temp = f.read()
videos = temp.split('\n')
...
# Create directory to hold test video frames
os.mkdir(f'{cd}/test_frames2')

# Loop to extract test video frames
for i in tqdm(range(test_videos.shape[0])):
    count = 0
    video_file = test_videos[i]
    cap = cv2.VideoCapture('test_videos2/' + video_file)
    frameRate = cap.get(5)
    x = 1
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        if frameId % math.floor(frameRate) == 0:
            # Store video frames in newly created 'test_frames2' directory
            filename = 'test_frames2/' + video_file[:-4] + "_frame%d.jpg" % count
            count += 1
            cv2.imwrite(filename, frame)
    cap.release()

    # Read frames from 'test_frames2' directory
    images = glob("test_frames2/*.jpg")
...
cd = os.getcwd()
os.mkdir(f'{cd}/frames3')
# Extract pass video frames
for i in range(1, 26):
    count = 0
    videoFile = f"{cd}/training_videos3/pass{i}.mp4"
    cap = cv2.VideoCapture(videoFile)
    frameRate = cap.get(5)
    x = 1
    while cap.isOpened():
        ...
        if frameId % math.floor(frameRate) == 0:
            ext = f'pass{i}_'
            filename = f"{cd}/frames3/{ext}frame%d.jpg" % count
            count += 1
            cv2.imwrite(filename, frame)
    cap.release()
    print("Done!")
...
for i in range(1, 26):
    count = 0
    videoFile = f"{cd}/training_videos3/run{i}.mp4"
    cap = cv2.VideoCapture(videoFile)
    frameRate = cap.get(cv2.CAP_PROP_FPS)
    x = 1
    while cap.isOpened():
...
# Read in mapping.csv
data = pd.read_csv(f'{cd}/mapping3.csv')
...
# Reshape to 1-D
X_train = X_train.reshape(346, 7 * 7 * 512)
X_valid = X_valid.reshape(116, 7 * 7 * 512)
...
# Get list of test file names
test_file_path = cd + r'\test_videos3'
f = open(test_file_path + '\\' + 'test_list.txt', 'r')
temp = f.read()
videos = temp.split('\n')
...
# Create directory to hold test video frames
os.mkdir(f'{cd}/test_frames3')

# Loop to extract test video frames
for i in tqdm(range(test_videos.shape[0])):
    count = 0
    video_file = test_videos[i]
    cap = cv2.VideoCapture('test_videos3/' + video_file)
    frameRate = cap.get(5)
    x = 1
    ...
    while cap.isOpened():
        ...
        if frameId % math.floor(frameRate) == 0:
            # Store video frames in newly created 'test_frames2' directory
            filename = 'test_frames3/' + video_file[:-4] + "_frame%d.jpg" % count
            count += 1
            cv2.imwrite(filename, frame)
    cap.release()

    # Read frames from 'test_frames2' directory
    images = glob("test_frames3/*.jpg")
